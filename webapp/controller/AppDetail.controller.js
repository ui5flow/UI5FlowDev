sap.ui.define([
  ...
  return Controller.extend("com.oprtnl.ui5locserv.controller.AppDetail", {
    ...
    onObjectMatched: function(oEvent) {
      ...
      // Set bigger size limits
      this.getView().getModel("bspApplicationsList").setSizeLimit(1000000);
      ...
      this.getView().getModel("applicationsList").attachEventOnce("requestCompleted", function(oEvent) {
        ...
        if (appListData.applications) {
          ...
          for (var i = 0, iLength = appListData.applications.length; i < iLength; i++) {
            ...
            if (appListData.applications[i].tags) {
              for (var j = 0, jLength = appListData.applications[i].tags.length; j < jLength; j++) {
                allTagsIndex[appListData.applications[i].tags[j].name] = true;
              }
            }
          ...
          appDetailData.all_tags = [];
          for (var key in allTagsIndex) {
            appDetailData.all_tags.push({
              "name": key
            });
          }
        }
    ...
    onDetailDataSave: function() {
      ...
      var oPostData = {
        ...
        "tags": this.getView().getModel("applicationDetail").getProperty("/tags")
      };
    ...
    onPathPress: function(oEvent) {
      ...
      if (!url) {
        this.showMessageToast({
          message: this.getI18nText("AppDetail.pathUrlCopyFailed")
        });
        return false;
      }
      ...
      try {
        ...
        this.showMessageToast({
          message: this.getI18nText("AppDetail.pathUrlCopySuccessful")
        });
      } catch (err) {
        this.showMessageToast({
          message: this.getI18nText("AppDetail.pathUrlCopyFailed")
    ...
    onBspUrlCopy: function(oEvent) {
      ...
      if (!url) {
        this.showMessageToast({
          message: this.getI18nText("AppDetail.bspUrlCopyFailed")
        });
        return false;
      }
      var textArea = document.createElement("textarea");
      ...
      try {
        var successful = document.execCommand("copy");
        this.showMessageToast({
          message: this.getI18nText("AppDetail.bspUrlCopySuccessful")
        });
      } catch (err) {
        this.showMessageToast({
          message: this.getI18nText("AppDetail.bspUrlCopyFailed")
    ...
    onVersionDeployAuthConfirm: function() {
      ...
      this.deploymentConfig["requestHeaders"] = {
        "Authorization": "Basic " + btoa(this.getInputValue("sapDeployUser") + ":" + this.getInputValue("sapDeployPassword"))
      };

      this.getView().byId("deploymentBasicAuth").setBusyIndicatorDelay(0);
      this.getView().byId("deploymentBasicAuth").setBusy(true);
      ...
      jQuery.ajax({
        ...
        success: function(oResponse) {
          ...
          that.getView().byId("deploymentBasicAuth").setBusy(false);
    ...
    getTransportsList: function() {
      ...
      this.getView().byId("versionDeployBsp").setBusy(true);
      this.getView().byId("versionDeployBsp").setBusyIndicatorDelay(0);
      ...
      jQuery.ajax({
        ...
        success: function(oResponse) {
          ...
          that.getView().byId("versionDeployBsp").setBusy(false);
    ...
    onVersionDeploySyncOk: function() {
      ...
      this.getView().byId("versionDeploySync").setBusy(true);
      this.getView().byId("versionDeploySync").setBusyIndicatorDelay(0);
      ...
      jQuery.ajax({
        ...
        success: function(oResponse) {
          ...
          that.getView().byId("versionDeploySync").setBusy(false);
    ...
    onNewAppTag: function(oEvent) {
      var tag = this.getView().byId("appAddTag").getValue();
      if (!tag) {
        return false;
      }
      var appData = this.getView().getModel("applicationDetail").getData();
      if (!appData.tags) {
        appData["tags"] = [];
      }

      // Do not add the same tags
      var tagExists = appData.tags.findIndex(function(obj) {
        return obj.name == tag;
      });
      ...
      this.getView().getModel("applicationDetail").refresh();
      this.getView().byId("appAddTag").setValue("");
    },

    onTagDelete: function(oEvent) {

      var removedTag = oEvent.getParameters("removedTokens");
      if (removedTag.type == "removed") {
        var tags = this.getView().byId("appTags").getTokens();

        if (oEvent.getParameters("removedTokens")) {
          var removedTagKey = removedTag.token.getProperty("key");
          var tagsData = this.getView().getModel("applicationDetail").getData().tags ? this.getView().getModel("applicationDetail").getData().tags : [];
          for (var i = 0, iLength = tagsData.length; i < iLength; i++) {
            if (tagsData[i].name === removedTagKey) {
              tagsData.splice(i, 1);
              break;
            }
          }
    ...
    onTagSuggestionSelected: function(oEvent) {
      ...
      var oSelectedItem = oEvent.getParameter("selectedItem");
      var tag = oSelectedItem.getProperty("text");
    ...
    onValueHelpTag: function(oEvent) {

      this.inputId = oEvent.getSource().getId();
      // create value help dialog
      if (!this.valueHelpTagilter) {
        this.valueHelpTagilter = sap.ui.xmlfragment(
          "com.oprtnl.ui5locserv.view.HelpTag",
          this
        );
        this.getView().addDependent(this.valueHelpTagilter);
        // forward compact/cozy style into dialog
        jQuery.sap.syncStyleClass(this.getView().getController().getContentDensityClass(), this.getView(), this.valueHelpTagilter);
      }

      this.valueHelpTagilter.getBinding("items").filter([]);

      // open value help dialog filtered by the input value
      this.valueHelpTagilter.open();
    },

    onValueHelpTagFilterClose: function(oEvent) {
      var oSelectedItem = oEvent.getParameter("selectedItem");
      if (oSelectedItem) {
        var inputObject = this.getView().byId(this.inputId);
        inputObject.setValue(oSelectedItem.getTitle());
    ...
    onValueHelpTagSearch: function(oEvent) {
